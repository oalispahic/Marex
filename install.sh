...
# Print one marex binary path per line for every directory on $PATH that
# contains an executable named 'marex'.  Using printf '%s\n' keeps paths
# with spaces intact.
find_marex_on_path() {
    local dir
    local saved_IFS="$IFS"
...
1)
    ...
    "$CXX" -std=c++14 -O2 "$SCRIPT_DIR"/src/*.cpp -I "$SCRIPT_DIR/include" -o "$BUILD_DIR/marex"
...
2)
    ...
    mapfile -t ALL_BINS < <(find_marex_on_path)
